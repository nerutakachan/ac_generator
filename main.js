...
// ==========================================
// ★ 【追加】dataフォルダへの一括上書き保存（自動バックアップ付き）
// ==========================================
const btnExportFolder = document.getElementById('btn-export-folder');
if (btnExportFolder) {
	btnExportFolder.addEventListener('click', async () => {
		// 1. dataフォルダのパスが記憶されているか確認
		const folderPath = window.currentProject?.environment?.data_folder;
		if (!folderPath) {
			alert("⚠️ dataフォルダのパスが記憶されていません。\n先に「dataフォルダを一括読み込み」ボタンからフォルダを指定してください。");
			return;
		}

		// 2. ユーザーへ最終確認
		const confirmExport = confirm(`【安全装置付き・上書き書き出し】\n現在エディターで編集中の数値を、指定されている下記のdataフォルダへ一括上書き保存しますか？\n\n対象パス:\n${folderPath}\n\n※元の本物ファイルは、自動的に「backup_data」フォルダへ1世代分100%安全にコピー（退避）されます。`);
		if (!confirmExport) return;

		console.log("💾 [FOLDER_EXPORT] 一括書き出し用のテキストジェネレートを開始します...");
		
		const originalText = btnExportFolder.textContent;
		btnExportFolder.textContent = "⏳ バックアップ＆書き出し中...";
		btnExportFolder.style.opacity = "0.5";
		btnExportFolder.style.pointerEvents = "none";

		try {
			// 3. アプリの記憶領域から、最新の数値をアセットコルサ専用のテキスト形式に逆変換（ジェネレート）する
			const fileDataMap = {};

			// 各種主要iniファイルのテキスト化
			if (window.currentSuspensionData) fileDataMap['suspensions.ini'] = stringifyINI(window.currentSuspensionData);
			if (window.currentTyreData)       fileDataMap['tyres.ini']       = stringifyINI(window.currentTyreData);
			if (window.currentAeroData)       fileDataMap['aero.ini']        = stringifyINI(window.currentAeroData);
			if (window.currentEngineData)     fileDataMap['engine.ini']      = stringifyINI(window.currentEngineData);
			if (window.currentSetupData)      fileDataMap['setup.ini']       = stringifyINI(window.currentSetupData);
			if (window.currentDrivetrainData) fileDataMap['drivetrain.ini']  = stringifyINI(window.currentDrivetrainData);

			// car.ini と colliders.ini の分離とテキスト化（マージされていたコライダーを元の2ファイルに綺麗に分ける）
			if (window.currentCarData) {
				const carDataClean = {};
				const colliderDataClean = {};
				
				for (const section in window.currentCarData) {
					if (section.startsWith('COLLIDER_')) {
						colliderDataClean[section] = window.currentCarData[section];
					} else {
						carDataClean[section] = window.currentCarData[section];
					}
				}
				fileDataMap['car.ini'] = stringifyINI(carDataClean);
				// コライダーデータが存在する場合のみファイル化
				if (Object.keys(colliderDataClean).length > 0) {
					fileDataMap['colliders.ini'] = stringifyINI(colliderDataClean);
				}
			}

			// パワー曲線（power.lut）のテキスト化
			if (window.currentPowerLutRaw) {
				fileDataMap['power.lut'] = stringifyLUT(window.currentPowerLutRaw);
			}

			// ギア比（final.rto）のテキスト化
			if (window.finalRtoList) {
				fileDataMap['final.rto'] = stringifyRTO(window.finalRtoList);
			}

			// カメラ（cameras.ini）のテキスト化
			if (window.cameraConfigs) {
				fileDataMap['cameras.ini'] = stringifyCameras(window.cameraConfigs);
			}

			// 4. ★裏側（main-electron.js）の安全装置へ、生成したテキストマップを丸ごと送信！
			if (!window.electronAPI || typeof window.electronAPI.saveAcDataFolder !== 'function') {
				throw new Error("window.electronAPI.saveAcDataFolder が見ていません。preload.jsを確認してください。");
			}

			const result = await window.electronAPI.saveAcDataFolder(folderPath, fileDataMap);
			
			if (result && result.success) {
				console.log("✨ [FOLDER_EXPORT] backup_dataへの退避および上書き保存が100%成功しました！");
				alert(`✅ dataフォルダへの書き出しが完了しました！\n\n元のファイルは「backup_data」フォルダ内へ安全に自動退避されました。エクスプローラー等でご確認ください。`);
			} else {
				alert("❌ 書き出しに失敗しました:\n" + (result?.error || "原因不明"));
			}

		} catch (err) {
			console.error("❌ [FOLDER_EXPORT] エクスポート中にクラッシュしました:", err);
			alert("書き出し処理中にエラーが発生しました。");
		} finally {
			btnExportFolder.textContent = originalText;
			btnExportFolder.style.opacity = "1";
			btnExportFolder.style.pointerEvents = "auto";
		}
	});
}

// ➔ これ以降のテキスト変換用ヘルパー関数群（INI / LUT / RTO / CAMERAの逆変換用）
...
function stringifyLUT(data) {
	if (!Array.isArray(data)) return "";
	return data.map(p => `${p.rpm}|${p.torque}`).join('\r\n') + '\r\n';
}

function stringifyRTO(data) {
	if (!Array.isArray(data)) return "";
	return data.map(p => `${p.name}|${p.value}`).join('\r\n') + '\r\n';
}

function stringifyCameras(configs) {
	if (!configs) return "";
	let text = "";
	const cList = Array.isArray(configs) ? configs : Object.values(configs);
	cList.forEach((c, i) => {
		text += `[CAMERA_${i}]\r\n`;
		if (c.x !== undefined && c.y !== undefined && c.z !== undefined) {
			text += `POINT=${c.x},${c.y},${c.z}\r\n`;
		}
		if (c.rx !== undefined && c.ry !== undefined && c.rz !== undefined) {
			text += `LOOK=${c.rx},${c.ry},${c.rz}\r\n`;
		}
		if (c.fov !== undefined) {
			text += `FOV=${c.fov}\r\n`;
		}
...
function processImportedFile(fileName, rawText, shouldUpdateUI = true) {
	...
	if (fileName.endsWith('.lut')) {
		...
		// ★追加：power.lut の名前をシステム専用キーに変換する
		if (fileName === 'power.lut') {
			stateKey = 'power_lut_raw';
		}
